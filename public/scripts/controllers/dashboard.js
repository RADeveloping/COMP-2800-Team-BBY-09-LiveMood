...
function getMonthScore(month) {
	...
	for (let dd = 1; dd <= daysInMonth; dd++) {
		...
		let userDayScoreRef = db.collection('users').doc(userId).collection('surveyTaken').doc(surveyId);
...
$.getJSON('https://api.covid19api.com/summary', function(data) {
	...
	if (data) {
		...
		let string =
			'As of date there has been a total of <strong>' +
			totalConfirmed +
			' </strong>' +
			'confirmed cases of COVID-19 and a total of <strong>' +
			TotalRecovered +
			'</strong> confirmed recoveries in Canada. <small>Data provided by: covid19api.com</small>';
